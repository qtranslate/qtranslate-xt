...
(function () {
  ...
  wp.apiFetch.use(function (options, next) {
    ...
    var typeData = wp.data.select('core').getPostType(post.type);
    ...
    var prefixPath = '/wp/v2/' + typeData.rest_base + '/' + post.id;

    if (options.path.startsWith(prefixPath) && options.method === 'PUT' || options.path.startsWith(prefixPath + '/autosaves') && options.method === 'POST') {

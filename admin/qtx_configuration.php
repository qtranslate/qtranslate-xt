...
function qtranxf_language_form($form_action, $button_name, $nonce_action) {
	global $q_config;
...
?>
<div class="form-wrap">
<form action="<?php echo $form_action ?>" id="qtranxs-edit-language" method="post" class="add:the-list: validate">
<input type="hidden" id="addlang_wpnonce" name="_wpnonce" value="<?php echo wp_create_nonce( $nonce_action ) ?>" />
...
<div class="form-field">
	...
	<p class="qtranxs-notes"><?php _e('Depending on your Date / Time Conversion Mode, you can either enter a <a href="http://www.php.net/manual/function.strftime.php">strftime</a> or <a href="http://www.php.net/manual/function.date.php">date</a> format. This field is optional. (Example: %I:%M %p)', 'qtranslate') ?></p>
</div>
<div class="form-field">
...
function qtranxf_license_form($form_action, $button_name, $nonce_action) {
	global $q_config;
?>
<div class="form-wrap"><form action="<?php echo $form_action ?>" id="qtranxs-edit-license" method="post" class="">
<input type="hidden" id="edit_license_wpnonce" name="_wpnonce" value="<?php echo wp_create_nonce( $nonce_action ) ?>" />
<?php wp_referer_field() ?>
</form></div>
<?php
}
...
function qtranxf_admin_section_end($nm, $button_name=null) {
	if($button_name !== false){
		if(is_null($button_name)) $button_name = __('Save Changes', 'qtranslate');
...
function qtranxf_get_admin_sections() {
	...
	//$admin_sections['license'] = __('License', 'qtranslate');//always last section
	...
	<?php qtranxf_admin_section_start('advanced') ?>
		<table class="form-table qtranxs-form-table" id="qtranxs_advanced_config">
			<tr>
				...
				<td>
					<p><?php _e('Post types enabled for translation:', 'qtranslate') ?></p><p>
					<?php
					...
					?>
					</p><p class="qtranxs-notes"><?php _e('If a post type unchecked, no fields in a post of that type are treated as translatable on editing pages. However, the manual raw multilingual entries with language tags may still get translated in a usual way at front-end.', 'qtranslate') ?></p>
				</td>
			</tr>
			<tr>
			...
			<tr  id="option_editor_mode">
				...
				<td>
					...
					<p class="qtranxs-notes"><?php echo __('This is the default mode.', 'qtranslate').' '.__('Pages with translatable fields have Language Switching Buttons, which control what language is being edited, while admin language stays the same.', 'qtranslate') ?></p><br/>
					...
					<p class="qtranxs-notes"><?php _e('Some people prefer to edit the raw entries containing all languages together separated by language defining tags, as they are stored in database.', 'qtranslate') ?></p><br/>
			...
			<tr  id="option_highlight_mode">
				...
				<td>
					...
					<p class="qtranxs-notes"><?php echo __('To reset to default, clear the text.', 'qtranslate').' '; printf(__('The color in use is taken from your profile option %s, the third color.', 'qtranslate'), '"<a href="'.admin_url('/profile.php').'">'.qtranxf_translate_wp('Admin Color Scheme').'</a>"') ?></p>
				</td>
			</tr>
<?php /*
			<tr>
		...
		<table class="form-table qtranxs-form-table" id="qtranxs_integration_config">
			...
			<tr>
				...
				<td><label for="qtranxs_text_field_filters" class="qtranxs_explanation">
					...
					<p class="qtranxs-notes"><?php printf(__('Names of filters (which are enabled on theme or other plugins via %s function) to add translation to. For more information, read %sFAQ%s.', 'qtranslate'),'apply_filters()','<a href="https://qtranslatexteam.wordpress.com/faq/#CustomFields">','</a>') ?></p>
				</td>
			</tr>
			<?php /* ?>
			<tr>
...
<?php
		if(!empty($admin_sections['license']))
			qtranxf_qtranxf_license_tab( $clean_uri, $nonce_action );
	}
?>
</div>
<?php
...
function qtranxf_qtranxf_license_tab($clean_uri, $nonce_action)
{
?>
<div class="tabs-content">
<?php
	qtranxf_admin_section_start('license');
	qtranxf_license_form($clean_uri, __('Save Changes &raquo;', 'qtranslate'), $nonce_action);
